/* eslint-disable @typescript-eslint/no-explicit-any */
import { FC, useEffect, useState } from 'react';
import { useIntl } from 'react-intl';
import { PageTitle } from '../../../../_metronic/layout/core';
import { KTIcon, KTSVG } from '../../../../_metronic/helpers';
import * as Yup from 'yup';
import { useFormik } from 'formik';
import clsx from 'clsx';

import '../GestionCustomer.css';
import { CustomerTypeRequest } from '../core/services/CustomerTypeRequests';
import { CustomerTypeModel } from '../core/models/CustomerType';
import { closeModal, handleHttpError } from '../../../utils/functions';
import { handleUserPermissionAccess } from '../../../utils/permissionAccessHandler';

const customerTypeSchema = Yup.object().shape({
  libelle: Yup.string().required('Libelle is required'),
});

const customerTypeRequest = new CustomerTypeRequest();

const CustomerType: FC = () => {
  const intl = useIntl();

  const initialValues = new CustomerTypeModel();

  const toUpdateValue = new CustomerTypeModel();

  const [toDeleteValue, settoDeleteValue] = useState<any>();

  const [loading, setLoading] = useState(false);
  const [customerTypesloader, setcustomerTypesloader] = useState(true);
  const [customerTypesDeleteloader, setcustomerTypesDeleteloader] =
    useState(false);
  const [customerTypesDatas, setcustomerTypesDatas] = useState(
    Array<CustomerTypeModel>,
  );

  const formik = useFormik({
    initialValues,
    validationSchema: customerTypeSchema,
    onSubmit: async (values) => {
      setLoading(true);
      console.log(values);

      await customerTypeRequest
        .addCustumerType(values)
        .then(() => {
          closeModal('modal_close-formik');
          getCustomerTypeList();
          setLoading(false);
        })
        .catch((error: any) => {
          handleHttpError(error);
        });
    },
  });

  const formikUpdateCustomerType = useFormik({
    initialValues: toUpdateValue,
    validationSchema: customerTypeSchema,
    enableReinitialize: true,
    onSubmit: async (values) => {
      setLoading(true);

      console.log(values);

      await customerTypeRequest
        .updateCustumerType(values)
        .then(() => {
          closeModal('modal_close-formikUpdateCustomerType');
          getCustomerTypeList();
          setLoading(false);
        })
        .catch((error: any) => {
          handleHttpError(error);
        });
    },
  });

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const selectToChange = (data: CustomerTypeModel | any) => {
    formikUpdateCustomerType.setValues(data);
  };

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const selectToDelete = (data: CustomerTypeModel | any) => {
    settoDeleteValue(data);
  };

  const getCustomerTypeList = async () => {
    setcustomerTypesloader(true);

    await customerTypeRequest
      .getCustumerTypeList(0, 10)
      .then((response: any) => {
        console.log(response.data);
        setcustomerTypesDatas(response.data.items);
        setcustomerTypesloader(false);
      })
      .catch((error: any) => {
        handleHttpError(error);
      });
  };

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const deleteCustomerType = async () => {
    setcustomerTypesDeleteloader(true);

    await customerTypeRequest
      .deleteCustumerType(toDeleteValue?.id)
      .then(() => {
        getCustomerTypeList();
        closeModal('modal_close-delete');
      })
      .catch((error: any) => {
        handleHttpError(error);
      });
  };

  useEffect(() => {
    getCustomerTypeList();
  }, []);

  return (
    <>
      <PageTitle breadcrumbs={[]}>
        {intl.formatMessage({ id: 'SETTINGS.CUSTOMER_TYPE' })}
      </PageTitle>

      {/* Add New Customer Type modal */}
      <div className="modal fade" tabIndex={-1} id="kt_modal_1">
        <div className="modal-dialog">
          <form
            className="form w-100"
            onSubmit={formik.handleSubmit}
            noValidate
            id="kt_login_signin_form"
          >
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">
                  {intl.formatMessage({ id: 'CUSTOMER.CUSTOMER_TYPE_ADDING' })}
                </h5>
                <div
                  className="btn btn-icon btn-sm btn-active-light-primary ms-2"
                  data-bs-dismiss="modal"
                  id="modal_close-formik"
                  aria-label="Close"
                >
                  <KTSVG
                    path="media/icons/duotune/arrows/arr061.svg"
                    className="svg-icon svg-icon-2x"
                  />
                </div>
              </div>

              <div className="modal-body">
                <div className="fv-row mb-8">
                  <label className="form-label fs-6 fw-bolder text-gray-900">
                    {intl.formatMessage({ id: 'GEN.LIBELLE' })}
                  </label>
                  <input
                    placeholder={intl.formatMessage({ id: 'GEN.LIBELLE' })}
                    {...formik.getFieldProps('libelle')}
                    className={clsx(
                      'form-control bg-transparent',
                      {
                        'is-invalid':
                          formik.touched.libelle && formik.errors.libelle,
                      },
                      {
                        'is-valid':
                          formik.touched.libelle && !formik.errors.libelle,
                      },
                    )}
                    type="text"
                    name="libelle"
                    autoComplete="on"
                  />
                  {formik.touched.libelle && formik.errors.libelle && (
                    <div className="fv-plugins-message-container">
                      <span role="alert">{formik.errors.libelle}</span>
                    </div>
                  )}
                </div>
              </div>

              <div className="modal-footer">
                <button
                  type="button"
                  className="btn btn-light"
                  data-bs-dismiss="modal"
                >
                  {intl.formatMessage({ id: 'GEN.CANCEL' })}
                </button>

                <button
                  type="submit"
                  className="btn btn-primary"
                  disabled={formik.isSubmitting || !formik.isValid}
                >
                  {!loading && (
                    <span className="indicator-label">
                      {intl.formatMessage({ id: 'AUTH.GENERAL.SUBMIT_BUTTON' })}
                    </span>
                  )}
                  {loading && (
                    <span
                      className="indicator-progress"
                      style={{ display: 'block' }}
                    >
                      {intl.formatMessage({ id: 'GEN.WAIT' })}
                      <span className="spinner-border spinner-border-sm align-middle ms-2"></span>
                    </span>
                  )}
                </button>
              </div>
            </div>
          </form>
        </div>
      </div>

      {/* updating Customer Type modal */}
      <div className="modal fade" tabIndex={-1} id="kt_modal_2">
        <div className="modal-dialog">
          <form
            className="form w-100"
            onSubmit={formikUpdateCustomerType.handleSubmit}
            noValidate
            id="kt_login_signin_form"
          >
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">
                  {intl.formatMessage({
                    id: 'CUSTOMER.CUSTOMER_TYPE_UPDATING',
                  })}
                </h5>
                <div
                  className="btn btn-icon btn-sm btn-active-light-primary ms-2"
                  data-bs-dismiss="modal"
                  id="modal_close-formikUpdateCustomerType"
                  aria-label="Close"
                >
                  <KTSVG
                    path="media/icons/duotune/arrows/arr061.svg"
                    className="svg-icon svg-icon-2x"
                  />
                </div>
              </div>

              <div className="modal-body">
                <div className="fv-row mb-8">
                  <label className="form-label fs-6 fw-bolder text-gray-900">
                    {intl.formatMessage({ id: 'GEN.LIBELLE' })}
                  </label>
                  <input
                    placeholder={intl.formatMessage({ id: 'GEN.LIBELLE' })}
                    {...formikUpdateCustomerType.getFieldProps('libelle')}
                    className={clsx(
                      'form-control bg-transparent',
                      {
                        'is-invalid':
                          formikUpdateCustomerType.touched.libelle &&
                          formikUpdateCustomerType.errors.libelle,
                      },
                      {
                        'is-valid':
                          formikUpdateCustomerType.touched.libelle &&
                          !formikUpdateCustomerType.errors.libelle,
                      },
                    )}
                    type="text"
                    name="libelle"
                    autoComplete="on"
                  />
                  {formikUpdateCustomerType.touched.libelle &&
                    formikUpdateCustomerType.errors.libelle && (
                      <div className="fv-plugins-message-container">
                        <span role="alert">
                          {formikUpdateCustomerType.errors.libelle}
                        </span>
                      </div>
                    )}
                </div>
              </div>

              <div className="modal-footer">
                <button
                  type="button"
                  className="btn btn-light"
                  data-bs-dismiss="modal"
                >
                  {intl.formatMessage({ id: 'GEN.CANCEL' })}
                </button>

                <button
                  type="submit"
                  className="btn btn-primary"
                  disabled={
                    formikUpdateCustomerType.isSubmitting ||
                    !formikUpdateCustomerType.isValid
                  }
                >
                  {!loading && (
                    <span className="indicator-label">
                      {intl.formatMessage({ id: 'AUTH.GENERAL.SUBMIT_BUTTON' })}
                    </span>
                  )}
                  {loading && (
                    <span
                      className="indicator-progress"
                      style={{ display: 'block' }}
                    >
                      {intl.formatMessage({ id: 'GEN.WAIT' })}
                      <span className="spinner-border spinner-border-sm align-middle ms-2"></span>
                    </span>
                  )}
                </button>
              </div>
            </div>
          </form>
        </div>
      </div>

      {/* delete Customer Type modal */}
      <div className="modal fade" tabIndex={-1} id="kt_modal_3">
        <div className="modal-dialog">
          <form className="form w-100" noValidate id="kt_login_signin_form">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">
                  {intl.formatMessage({
                    id: 'CUSTOMER.CUSTOMER_TYPE_DELETING',
                  })}
                </h5>
                <div
                  className="btn btn-icon btn-sm btn-active-light-primary ms-2"
                  data-bs-dismiss="modal"
                  id="modal_close-delete"
                  aria-label="Close"
                >
                  <KTSVG
                    path="media/icons/duotune/arrows/arr061.svg"
                    className="svg-icon svg-icon-2x"
                  />
                </div>
              </div>

              <div className="modal-body">
                <div className="fv-row mb-8">
                  <h3 className="fw-bolder text-center col-12 mv-2rem">
                    {intl.formatMessage({
                      id: 'CUSTOMER.CUSTOMER_TYPE_WOULD_YOU_WANNA_DELETE',
                    })}{' '}
                    :{' '}
                    <span className="text-danger">
                      {toDeleteValue?.libelle}{' '}
                    </span>{' '}
                    ?
                  </h3>
                </div>
              </div>

              <div className="modal-footer">
                <button
                  type="button"
                  className="btn btn-light"
                  data-bs-dismiss="modal"
                >
                  {intl.formatMessage({ id: 'GEN.CANCEL' })}
                </button>

                <button
                  type="submit"
                  className="btn btn-danger"
                  onClick={() => deleteCustomerType()}
                >
                  {!customerTypesDeleteloader && (
                    <span className="indicator-label">
                      {intl.formatMessage({
                        id: 'CUSTOMER.CUSTOMER_TYPE_DELETE',
                      })}
                    </span>
                  )}
                  {customerTypesDeleteloader && (
                    <span
                      className="indicator-progress"
                      style={{ display: 'block' }}
                    >
                      {intl.formatMessage({ id: 'GEN.WAIT' })}
                      <span className="spinner-border spinner-border-sm align-middle ms-2"></span>
                    </span>
                  )}
                </button>
              </div>
            </div>
          </form>
        </div>
      </div>

      {/* Customer Types list */}
      <div className="row g-5 gx-xxl-12">
        <div className="col-xxl-12">
          <div className={`card card-xxl-stretch mb-5 mb-xxl-8`}>
            <div className="card-header border-0 pt-5">
              <h5 className="card-title align-items-start flex-column">
                <span className="card-label fw-bold fs-3 mb-1">
                  {/* {intl.formatMessage({id: 'GEN.lIST_OF'})}
                        {intl.formatMessage({id: 'SETTINGS.CUSTOMER_TYPE'})} */}
                </span>
                {/* <span className='text-muted mt-1 fw-semibold fs-7'>More than 400 new products</span> */}
              </h5>

              <div className="card-toolbar">
                <ul className="nav">
                  <li className="nav-item">
                    <button
                      type="button"
                      className="btn btn-primary"
                      data-bs-toggle="modal"
                      data-bs-target="#kt_modal_1"
                      data-permission-guard={handleUserPermissionAccess(
                        'customer_type:create',
                      )}
                    >
                      {intl.formatMessage({ id: 'CUSTOMER.CUSTOMER_TYPE_ADD' })}
                    </button>
                  </li>
                </ul>
              </div>
            </div>

            <div className="card-body py-3">
              <div className="">
                <div className="" id="">
                  {/* begin::Table container */}
                  <div className="table-responsive">
                    <table className="table table-row-dashed table-row-gray-200 align-middle gs-0 gy-4">
                      <thead>
                        <tr className="border-0">
                          <th className="p-0 w-50px fw-bolder text-left">N°</th>
                          <th className="p-0 min-w-150px fw-bolder text-left">
                            Libelle
                          </th>
                          <th className="p-0 min-w-50px fw-bolder text-left">
                            Action
                          </th>
                        </tr>
                      </thead>

                      <tbody>
                        {customerTypesDatas?.map((row, index) => (
                          <tr key={row.id}>
                            <td className=" text-left">
                              <div className="symbol symbol-45px me-2">
                                <h5>#{index + 1}</h5>
                              </div>
                            </td>

                            <td className=" text-left">
                              <div className="symbol symbol-45px me-2 text-hover-primary">
                                <h5>{row.libelle}</h5>
                              </div>
                            </td>

                            <td className=" text-left">
                              <a
                                onClick={() => selectToChange(row)}
                                data-bs-toggle="modal"
                                data-bs-target="#kt_modal_2"
                                title={intl.formatMessage({
                                  id: 'CUSTOMER.CUSTOMER_TYPE_UPDATE',
                                })}
                                className="btn btn-sm btn-icon btn-bg-light btn-color-warning"
                                data-permission-guard={handleUserPermissionAccess(
                                  'customer_type:update',
                                )}
                              >
                                <KTIcon
                                  iconName="notepad-edit"
                                  className="fs-1"
                                />
                              </a>
                              <a
                                onClick={() => selectToDelete(row)}
                                data-bs-toggle="modal"
                                data-bs-target="#kt_modal_3"
                                title={intl.formatMessage({
                                  id: 'CUSTOMER.CUSTOMER_TYPE_DELETE',
                                })}
                                className="btn btn-sm btn-icon btn-bg-light btn-color-danger"
                                data-permission-guard={handleUserPermissionAccess(
                                  'customer_type:delete',
                                )}
                              >
                                <KTIcon iconName="trash" className="fs-1" />
                              </a>
                            </td>
                          </tr>
                        ))}
                      </tbody>
                      {/* end::Table body */}
                    </table>

                    <div className="row">
                      {customerTypesloader && customerTypesDatas.length == 0 ? (
                        <h3 className="fw-bolder text-center col-12 mv-2rem">
                          {intl.formatMessage({ id: 'GEN.LOADING' })}
                        </h3>
                      ) : (
                        ''
                      )}

                      {!customerTypesloader &&
                      customerTypesDatas.length == 0 ? (
                        <h3 className="fw-bolder text-center text-danger col-12 mv-2rem">
                          🚫 {intl.formatMessage({ id: 'GEN.NO_DATAS' })}
                        </h3>
                      ) : (
                        ''
                      )}
                    </div>
                  </div>
                  {/* end::Table */}
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </>
  );
};

export { CustomerType };
