...
const insurranceTypeSchema = Yup.object().shape({
  libelle: Yup.string().required('Libelle is required'),
});

const insurranceTypeRequest = new InsurranceTypeRequest();

const InsurranceType: FC = () => {
  const intl = useIntl();

  const initialValues = new InsurranceTypeModel();

  const toUpdateValue = new InsurranceTypeModel();

  const [toDeleteValue, settoDeleteValue] = useState<
    InsurranceTypeModel | any
  >();

  const [loading, setLoading] = useState(false);
  const [InsurranceTypeloader, setInsurranceTypeloader] = useState(true);
  const [InsurranceTypeDeleteloader, setInsurranceTypeDeleteloader] =
    useState(false);
  ...
  const formik = useFormik({
    initialValues,
    validationSchema: insurranceTypeSchema,
    onSubmit: async (values, { setStatus, setSubmitting }) => {
      setLoading(true);
      console.log(values);

      await insurranceTypeRequest
        .addInsurranceType(values)
        .then((response: any) => {
          closeModal('modal_close-initialValues');
          setSubmitting(false);
          setLoading(false);
  ...
  const formikUpdateInsurranceType = useFormik({
    ...
    onSubmit: async (values, { setStatus, setSubmitting }) => {
      ...
      await insurranceTypeRequest
        .updateInsurranceType(values)
        .then((response: any) => {
          closeModal('modal_close-formikUpdateInsurranceType');
          setSubmitting(false);
          setLoading(false);
          getInsurranceTypeList();
          formikUpdateInsurranceType.resetForm();
        })
        .catch((error: any) => {
          handleHttpError(error);
        });
  ...
  const selectToChange = (data: InsurranceTypeModel | any) => {
    formikUpdateInsurranceType.setValues(data);
  };

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const selectToDelete = (data: InsurranceTypeModel | any) => {
    settoDeleteValue(data);
  };

  const getInsurranceTypeList = async () => {
    setInsurranceTypeloader(true);

    await insurranceTypeRequest
      .getInsurranceTypeList()
      .then((response: any) => {
        setInsurranceTypeDatas(response.data.items);
        setInsurranceTypeloader(false);
      })
      .catch((error: any) => {
        handleHttpError(error);
      });
  ...
  const deleteComplaintCategory = async () => {
    setInsurranceTypeDeleteloader(true);

    await insurranceTypeRequest
      .deleteInsurranceType(toDeleteValue?.id)
      .then((response: any) => {
        getInsurranceTypeList();
        setInsurranceTypeDeleteloader(false);
  ...
  return (
    <>
            ...
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">
                  {intl.formatMessage({
                    id: 'TYPE_ASURRANCE.TYPE_ASURRANCE_ADDING',
                  })}
                </h5>
                <div
      ...
      {/* updating Insurrance Type modal */}
      <div className="modal fade" tabIndex={-1} id="kt_modal_2">
        <div className="modal-dialog">
          <form
            className="form w-100"
            onSubmit={formikUpdateInsurranceType.handleSubmit}
            noValidate
            id="kt_login_signin_form"
          >
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">
                  {intl.formatMessage({
                    id: 'TYPE_ASURRANCE.TYPE_ASURRANCE_UPDATING',
                  })}
                </h5>
                <div
                  className="btn btn-icon btn-sm btn-active-light-primary ms-2"
                  data-bs-dismiss="modal"
                  id="modal_close-formikUpdateInsurranceType"
                  aria-label="Close"
                >
                  <KTSVG
                    path="media/icons/duotune/arrows/arr061.svg"
              ...
              <div className="modal-body">
                <div className="fv-row mb-8">
                  ...
                  <input
                    placeholder={intl.formatMessage({ id: 'GEN.LIBELLE' })}
                    {...formikUpdateInsurranceType.getFieldProps('libelle')}
                    className={clsx(
                      'form-control bg-transparent',
                      {
                        'is-invalid':
                          formikUpdateInsurranceType.touched.libelle &&
                          formikUpdateInsurranceType.errors.libelle,
                      },
                      {
                        'is-valid':
                          formikUpdateInsurranceType.touched.libelle &&
              ...
              <div className="modal-footer">
                ...
                <button
                  ...
                  className="btn btn-primary"
                  disabled={
                    formikUpdateInsurranceType.isSubmitting ||
                    !formikUpdateInsurranceType.isValid
                  }
                >
                  {!loading && (
                    <span className="indicator-label">
          ...
          <form className="form w-100" noValidate id="kt_login_signin_form">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">
                  {intl.formatMessage({
                    id: 'TYPE_ASURRANCE.TYPE_ASURRANCE_DELETING',
                  })}
                </h5>
                <div
                  className="btn btn-icon btn-sm btn-active-light-primary ms-2"
              ...
              <div className="modal-body">
                <div className="fv-row mb-8">
                  <h3 className="fw-bolder text-center col-12 mv-2rem">
                    {intl.formatMessage({
                      id: 'TYPE_ASURRANCE.TYPE_ASURRANCE_WOULD_YOU_WANNA_DELETE',
                    })}{' '}
                    :{' '}
              ...
              <div className="modal-footer">
                ...
                >
                  {!InsurranceTypeDeleteloader && (
                    <span className="indicator-label">
                      {intl.formatMessage({
                        id: 'TYPE_ASURRANCE.TYPE_ASURRANCE_DELETE',
                      })}
                    </span>
                  )}
                  {InsurranceTypeDeleteloader && (
                  ...
                  <li className="nav-item">
                    <ACTION_LONG_BUTTON
                      ...
                      text={intl.formatMessage({
                        id: 'TYPE_ASURRANCE.TYPE_ASURRANCE_ADD',
                      })}
                      permission="insurance_type:create"
              ...
              <div className="">
                <div className="" id="">
                  <div className="table-responsive">
                            ...
                            <td className=" text-left">
                              <ACTION_LIST_ROW_MINI_BUTTON
                                ...
                                title={intl.formatMessage({
                                  id: 'TYPE_ASURRANCE.TYPE_ASURRANCE_UPDATE',
                                })}
                                permission="insurance_type:update"
                                icon="notepad-edit"
                              ...
                              <ACTION_LIST_ROW_MINI_BUTTON
                                ...
                                permission="insurance_type:delete"
                                icon="trash"
                              ...
                              {/* <a  onClick={() => selectToChange(row)}
                                                ...
                                                title={intl.formatMessage({id: 'TYPE_ASURRANCE.TYPE_ASURRANCE_UPDATE'})}
                                            ...
                                            <a
                                                ...
                                                title={intl.formatMessage({id: 'TYPE_ASURRANCE.TYPE_ASURRANCE_DELETE'})}
                    ...
                    <div className="row">
                      {InsurranceTypeloader &&
                      InsurranceTypeDatas.length == 0 ? (
                        <h3 className="fw-bolder text-center col-12 mv-2rem">
