...
const DashboardAsHolding: FC = () => {
  ...
  const [
    claimsStatsRateForAllInstitutionLoader,
    setclaimsStatsRateForAllInstitutionLoader,
  ] = useState(true);

  const [AllInstitutionLoader, setAllInstitutionLoader] = useState(true);
  ...
  const [rowsPerPageSize, setRowsPerPage] = useState(3);
  const [rowsPerPageOptions] = useState([10, 30, 60, 100]);
  const metricSlots = new Array(8);
  ...
  const [
    selectedMonthByForSatisfactionByStatus,
    setselectedMonthByForSatisfactionByStatus,
  ] = useState(new Date().getMonth() + 1);

  const [
    selectedYearForMetricsByStatusMonthly,
    setselectedYearForMetricsByStatusMonthly,
  ] = useState(new Date().getFullYear());

  const [selectedMonthForObjectStats, setselectedMonthForObjectStats] =
    useState(new Date().getMonth() + 1);

  const [selectedYearForObjectsStats, setselectedYearForObjectsStats] =
    useState(new Date().getFullYear());

  const [gettingObjectsStatsLoader, setgettingObjectsStatsLoader] =
    useState(true);

  const [claimsStatsOfObjects, setclaimsStatsOfObjects] = useState<any>();
  ...
  const [claimsStatsOfStatusByMonth, setclaimsStatsOfStatusByMonth] =
    useState<any>();
  const [
    claimsStatsRateForAllInstitution,
    setclaimsStatsRateForAllInstitution,
  ] = useState<any>();
  ...
  const metricPlaceHolderStekelonsLoaders = new Array(8);
      ...
      .then((response: any) => {
        ...
        setclaimsStatsOfStatusByMonth(
          _statusLabels.map((_label) => {
            return {
              name: intl.formatMessage({ id: `CLAIMS_STATUS.${_label}` }),
  ...
  const getClaimsStatsByReceptionCanal = async (month: number) => {
    setselectedMonthByForSatisfactionByStatus(month);
  ...
  const getClaimsStatsRateForAllInstitution = async () => {
    setclaimsStatsRateForAllInstitutionLoader(true);
    await statsRequest
      .getClaimsStatsRateForAllInstitution()
      .then((response: any) => {
        setTimeout(() => {
          console.log(response.data);
          setclaimsStatsRateForAllInstitution(response.data);
          setclaimsStatsRateForAllInstitutionLoader(false);
        }, 200);
      })
      .catch((error: any) => {
        handleHttpError(error);
  ...
  const getAllInstitutions = async () => {
    await institutionRequest
      ...
      .then((response: any) => {
        ...
        setAllInstitutions(response.data.items);
        setAllInstitutionLoader(false);
      })
      .catch((error: any) => {
        handleHttpError(error);
      });
  ...
  const getClaimsStatsByRObjectsTimePeriode = async (
    month: number,
    year: number,
  ) => {
  ...
  const getAllUnities = async () => {
    await unityRequest
      ...
      .then((response: any) => {
        console.log(response.data.items);
        setAllUnities(response.data.items);
      })
      .catch((error: any) => {
        handleHttpError(error);
      });
  ...
  const getClaimsStatsForTheWholeAPP = async (institutionId?: number) => {
    ...
    if (institutionId) {
      ...
      await statsRequest
        .getClaimsStatsForInstitution(datePeriode, institutionId)
        .then((response: any) => {
          console.log(response.data);
      ...
      await statsRequest
        .getClaimsStatsForWholeApp(datePeriode)
        .then((response: any) => {
          console.log(response.data);
          setclaimsStats(response.data);
          setclaimStatsLoader(false);
  ...
  useEffect(() => {
    ...
    getClaimsStatsByRObjectsTimePeriode(
      selectedMonthForObjectStats,
      selectedYearForObjectsStats,
    );
  ...
  return (
    <>
      <div className="row mt-5 mb-15">
        ...
        <button
          type="button"
          className="btn btn-light-primary me-3 col-2"
        ...
        {AllInstitutionLoader ? (
          <LOADER_INPUTLOADER></LOADER_INPUTLOADER>
        ) : (
          <>
            <select
              onChange={(e: any) =>
                getClaimsStatsForTheWholeAPP(e.target.value)
              }
            ...
            >
              ...
              {AllInstitutions?.map((row, index) => (
                <option key={index} value={row.id}>
                  {row.institution_type.libelle} : {row.name}
                </option>
              ))}
            </select>
          </>
        ...
        >
          ...
          <div className="px-7 py-5" data-kt-user-table-filter="form">
              ...
              <div className="mb-10" key={index}>
                <div className="form-check form-check-custom form-check-solid">
                  <input
                    ...
                    value={row}
                    id={'flexRadioChecked-' + row}
                  />
                  <label
                    className="form-check-label"
                    htmlFor={'flexRadioChecked-' + row}
                  >
                    {row} Dernier Jours
                  </label>
                </div>
            ...
            <div className="d-flex justify-content-end">
              ...
              <button
                onClick={() => getClaimsStatsForTheWholeAPP()}
                type="button"
      ...
      <div className="row g-5 g-xl-10">
        <div className="col-12">{claimStatsLoader && <LinearProgress />}</div>
        ...
        <div className="col-12 col-md-3">
          ...
          >
            <h4 style={{ textAlign: 'center' }}>
              {intl.formatMessage({
                id: 'CLAIM.STATS.TREATED_IN_TIMES_CLAIMS',
              })}
            </h4>
            <div className="d-flex">
              <div
              ...
              >
                {claimsStats ? claimsStats.nb_processed_in_delay_claims : '--'}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="col-12 col-md-3">
          ...
          >
            <h4 style={{ textAlign: 'center' }}>
              {intl.formatMessage({ id: 'CLAIM.STATS.SATISFACTION_NUMBERS' })}
            </h4>
            <div className="d-flex">
              <div
                className="col-6 metric_value"
              ...
              >
                {claimsStats ? claimsStats.nb_surveys : '--'}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="col-12 col-md-3">
          ...
          >
            <h4 style={{ textAlign: 'center' }}>
              {intl.formatMessage({ id: 'CLAIM.STATS.SATISFACTION_RATE' })}
            </h4>
            <div className="d-flex">
              <div
                className="col-6 metric_value"
      ...
      <div className="row g-5 g-xl-10 mt-8">
        ...
        <div className="col-12 col-md-12">
          {claimsStatsOfStatusByMonthLoader &&
          claimsStatsOfStatusByMonth?.length == 0 ? (
                ...
                <div className="card-toolbar">
                  ...
                  >
                    <input
                      ...
                      onChange={(e: any) =>
                        getClaimsStatsOfStatusByMonth(e.target.value)
                      }
                ...
                <div className="card-toolbar">
                  ...
                  >
                    <select
                      ...
                      data-hide-search="true"
                      onChange={(e) =>
                        getClaimsStatsByReceptionCanal(parseInt(e.target.value))
                      }
              ...
              <div className="card-body pt-2 pb-4 d-flex">
                <div className="row w-100">
                  <DashCardsWidgetBarChart
                    optionsLabels={Object.keys(
                      claimsStatsMetricsByReceptionCanal,
                    )}
                    optionsDatas={Object.values(
                      claimsStatsMetricsByReceptionCanal,
                    )}
                  />
                </div>
            ...
            <div className={`card card-flush`} style={{ height: '100%' }}>
              <div className="card-header border-0 pt-6">
                <div className="card-title">
                  <div className="d-flex align-items-center position-relative my-1">
                    {intl.formatMessage({
                      id: 'CLAIM.STATS.CLAIM_RATE_BY_FILIALE',
                    })}
                  </div>
                </div>
              </div>
              ...
              >
                <div className="me-5 pt-2">
                  <DashCardsWidgetDonutChart
                    chartType="pie"
                    hasLegend={true}
                    width="500"
                    height="400"
                    optionsLabels={Object.keys(
                      claimsStatsRateForAllInstitution.nb_claims_by_institution,
                    )}
                    optionsDatas={Object.values(
                      claimsStatsRateForAllInstitution.nb_claims_by_institution,
                    )}
                ...
                <div className="card-toolbar">
                  ...
                  >
                    <input
                      type="number"
                      onChange={(e: any) =>
                        changeYearObjectsTimePeriodeStatsval(e.target.value)
                      }
                    ...
                    <select
                      ...
                      data-hide-search="true"
                      onChange={(e) =>
                        changeMonthObjectsTimePeriodeStatsval(
