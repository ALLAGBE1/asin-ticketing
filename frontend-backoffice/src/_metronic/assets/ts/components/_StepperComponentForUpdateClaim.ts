...
class StepperComponentForUpdateClaim {
  element: HTMLElement;
  ...
  constructor(_element: HTMLElement, options: IStepperOptions) {
    ...
    // Variables
    this.totalStepsNumber = this.steps?.length | 4;
  ...
  public static getInstance(
    element: HTMLElement,
  ): StepperComponentForUpdateClaim | undefined {
    if (
      element !== null &&
      StepperComponentForUpdateClaim.hasInstace(element)
    ) {
      const data = DataUtil.get(element, 'stepper');
      if (data) {
        return data as StepperComponentForUpdateClaim;
      }
    }
  }
  ...
  public static createInstances(selector: string): void {
    ...
    elements.forEach((element) => {
      ...
      let stepper = StepperComponentForUpdateClaim.getInstance(item);
      if (!stepper) {
        stepper = new StepperComponentForUpdateClaim(
          item,
  ...
  ): StepperComponentForUpdateClaim | null => {
    if (!element) {
      return null;
    }
    let stepper = StepperComponentForUpdateClaim.getInstance(element);
    if (!stepper) {
      stepper = new StepperComponentForUpdateClaim(element, options);
    }
    return stepper;
  };

  public static bootstrap(attr: string = '[data-kt-stepper]') {
    StepperComponentForUpdateClaim.createInstances(attr);
  }
}

export { StepperComponentForUpdateClaim, defaultStepperOptions };
