...
const Header = () => {
  ...
  const [isLoggedIn, setIsLoggedIn] = useState(false); // État de connexion
    ...
    useEffect(() => {
        // Vérifie si un token est présent dans le localStorage au montage du composant
        const token = localStorage.getItem("token");
        setIsLoggedIn(!!token); // Met à jour l'état de connexion en fonction de la présence du token
    }, []);
  const Navbar = [
    {
      name: "Recherche",
    ...
    const handleLogout = () => {
        // Supprime le token et les infos de localStorage
          ...
          >
            <ul className="list-none lg:flex md:flex sm:block block gap-x-5 gap-y-16">
              {Navbar.map((item, index) => (
                ...
                {/* Afficher "Se connecter" si non connecté, "Déconnecter" si connecté */}
                {isLoggedIn ? (
                    <button
                        className="bg-customBlue text-[1.1rem] font-normal text-white px-5 py-1.5 rounded lg:ml-10 md:ml-6 sm:ml-0 ml-0"
